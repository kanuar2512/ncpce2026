...
const HEROICON_PATHS = Object.freeze({
  ...
  'presentation-chart-bar': '<path d="M3.75 3v11.25A2.25 2.25 0 006 16.5h2.25M3.75 3h-1.5m1.5 0h16.5m0 0h1.5m-1.5 0v11.25A2.25 2.25 0 0118 16.5h-2.25m-7.5 0h7.5m-7.5 0l-1 3m8.5-3l1 3m0 0l.5 1.5m-.5-1.5h-9.5m0 0l-.5 1.5m.75-9l3-3 2.148 2.148A12.061 12.061 0 0116.5 7.605"/>',
});
...
function speakerCardHTML(sp, lang, i) {
  ...
  const slides   = (sp.slide_url || '').trim();
  ...
  return `
    <div class="speaker-card reveal" data-delay="${Math.min(i + 1, 5)}">
      ...
      <div class="speaker-card__body">
        ...
        ${slides ? `<a class="speaker-card__cv speaker-card__slides" href="${slides}" target="_blank" rel="noopener" aria-label="Slaid Pembentangan / Presentation Slides">${hicon('presentation-chart-bar')}<span data-lang="ms">Slaid Pembentangan</span><span data-lang="en">Presentation Slides</span></a>` : ''}
      </div>
    </div>`;
}
